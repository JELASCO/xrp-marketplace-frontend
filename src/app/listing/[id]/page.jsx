            ...
            <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:16}}>
              ...
              {STEPS.map((step,i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                  <div style={{width:24,height:24,borderRadius:'50%',border:`2px solid ${i<escrowStep?'var(--green)':i===escrowStep?'var(--accent)':'#2d3748'}`,background:i<escrowStep?'var(--green)':'transparent',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,color:i<escrowStep?'#fff':i===escrowStep?'var(--accent)':'var(--text3)',fontWeight:700}}>{i<escrowStep?'✓':i+1}</div>
                  <span style={{color:i<escrowStep?'var(--green)':i===escrowStep?'var(--accent)':'var(--text3)',fontSize:13}}>{step}</span>
                </div>
              ))}
              {escrowStep===2 && (
                <div style={{marginTop:8,display:'flex',flexDirection:'column',gap:8}}>
                  <button onClick={handleConfirm} disabled={buying} style={{width:'100%',padding:10,borderRadius:8,border:'none',background:buying?'#065f46':'var(--green)',color:'#fff',fontWeight:700,cursor:buying?'not-allowed':'pointer'}}>
              ...
              {escrowStep>=4 && !reviewSubmitted && (
                <div style={{marginTop:16,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:10,padding:16}}>
                  ...
                  <button onClick={handleReview} disabled={!rating} style={{marginTop:8,width:'100%',padding:'10px',borderRadius:8,border:'none',background:rating?'var(--accent)':'#1e293b',color:'#fff',fontWeight:700,cursor:rating?'pointer':'not-allowed',fontSize:14}}>
                    Submit Review
                  </button>
                </div>
              )}
