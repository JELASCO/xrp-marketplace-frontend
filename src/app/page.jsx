...
export default function HomePage() {
  ...
  const tickerItems = () => {
    ...
    listings.slice(0,5).forEach((l, i) => out.push(
      <span key={'nl' + i}><span style={{color:M}}>NEW LISTING </span><span style={{color:V,fontWeight:600}}>{l.title}</span><span style={{color:M}}> · {Number(l.price_xrp).toLocaleString('en-US')} XRP</span></span>
    ));
    feed.filter(e => e.kind === 'released').forEach((e, i) => out.push(
      <span key={'sold' + i}><span style={{color:M}}>SOLD </span><span style={{color:V,fontWeight:600}}>{amt(e.xrp)} </span><span style={{color:G}}>XRP</span><span style={{color:M}}> · {ago(e.at)}</span></span>
    ));
  ...
  return (
    <div className="xh-home">{showLogin && <XummLoginModal onClose={()=>setShowLogin(false)} />}
      <style>{`
        ...
        .xh-escrow{--ec-bg:#fff;--ec-fg:#3D5A80;--ec-ink:#0A1628;--ec-sub:#3D5A80;--ec-mut:#7C92B2;--ec-line:rgba(10,35,66,.18);--ec-border:1px solid rgba(10,35,66,.12);--ec-shadow:0 1px 2px rgba(10,35,66,.05),0 24px 50px -24px rgba(10,35,66,.25);--ec-tile-bg:#EAF1FB;--ec-tile-bd:rgba(10,35,66,.12);--ec-tile-fg:#2080F5;--ec-done-bg:rgba(22,163,74,.12);--ec-done-bd:rgba(22,163,74,.30);--ec-done-fg:#16a34a;--ec-lbl:#7C92B2;--ec-lbl-b:#0A1628;--ec-sail:#000000;--ec-wave1:#d9e8fb;--ec-wave2:#c4dbf7;--ec-foot-bg:#F8FBFF;--ec-foot-bd:rgba(10,35,66,.12);--ec-foot-fg:#7C92B2;--ec-foot-link:#2080F5}
      ...
      <div style={{marginBottom:48}}>
        ...
        {loading ? (
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:12}}>
            {listings.slice(0,8).map(l => <ListingCard key={l.id} listing={l}/>)}{listings.length < 4 && (user ? <Link href="/listings/new" style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',gap:8,padding:'18px 14px',minHeight:215,border:'1.5px dashed var(--xh-border2, rgba(0,0,0,0.10))',borderRadius:14,background:'var(--xh-bg2, #fafbfc)',textDecoration:'none',cursor:'pointer'}}><div style={{width:40,height:40,borderRadius:12,background:'rgba(32,128,245,0.1)',color:'var(--xh-accent)',display:'grid',placeItems:'center',fontSize:22,fontWeight:300,lineHeight:1}}>+</div><div style={{fontSize:13.5,fontWeight:600,color:'var(--xh-text)'}}>List your item here</div><div style={{fontSize:11.5,color:'var(--xh-text3)',lineHeight:1.4}}>Few listings so far — be one of the first. 0 fees.</div></Link> : <div onClick={()=>setShowLogin(true)} style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',gap:8,padding:'18px 14px',minHeight:215,border:'1.5px dashed var(--xh-border2, rgba(0,0,0,0.10))',borderRadius:14,background:'var(--xh-bg2, #fafbfc)',textDecoration:'none',cursor:'pointer'}}><div style={{width:40,height:40,borderRadius:12,background:'rgba(32,128,245,0.1)',color:'var(--xh-accent)',display:'grid',placeItems:'center',fontSize:22,fontWeight:300,lineHeight:1}}>+</div><div style={{fontSize:13.5,fontWeight:600,color:'var(--xh-text)'}}>List your item here</div><div style={{fontSize:11.5,color:'var(--xh-text3)',lineHeight:1.4}}>Few listings so far — be one of the first. 0 fees.</div></div>)}
          </div>
        )}
      </div>
      ...
      <div style={{background:'linear-gradient(135deg, #0b1b33 0%, #142b52 55%, #1d4ed8 100%)',borderRadius:18,padding:'48px 28px 56px',textAlign:'center',marginBottom:24,color:'#fff',position:'relative',overflow:'hidden'}}>
        ...
        <p style={{fontSize:15,color:'#bcd2f7',maxWidth:520,margin:'0 auto 24px',lineHeight:1.55,position:'relative',zIndex:1}}>{user ? 'List your first item and start trading in under a minute.' : 'Connect your Xaman wallet and start trading in under a minute.'}</p>
