...
export default function ProfilePage() {
  ...
  const avatar = user.avatar_url || (currentUser && String(currentUser.id) === String(id) ? currentUser.avatar_url : null);
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '28px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
        <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#3b82f6,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, fontWeight: 700, color: '#fff', flexShrink: 0, overflow: 'hidden' }}>
          {avatar
            ? <img src={avatar} alt={user.username} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none'; }} />
            : initials}
        </div>
