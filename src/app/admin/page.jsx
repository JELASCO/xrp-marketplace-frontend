...
export default function AdminPage() {
  ...
  const removeListing = async (id) => {
    if (!confirm('Bu listing\'i yayından kaldırmak istediğine emin misin?')) return;
    setRemoving(r => ({ ...r, [id]: true }));
    try {
      await api.admin.removeListing(id);
      setListings(ls => ls.filter(l => l.id !== id));
      if (stats) setStats(s => ({ ...s, activeListings: s.activeListings - 1 }));
    } catch (e) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {listings.map(l => (
            <div key={l.id} style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              {(!l.images || !l.images[0]) && (
                <div style={{ width: 48, height: 48, borderRadius: 8, background: '#0a0e1a', flexShrink: 0 }} />
              )}
              ...
              <button
                ...
                style={{ flexShrink: 0, padding: '7px 14px', fontSize: 12, fontWeight: 600, cursor: removing[l.id] ? 'not-allowed' : 'pointer', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, background: 'rgba(239,68,68,0.08)', color: removing[l.id] ? '#4a5568' : '#f87171', fontFamily: 'inherit', transition: 'all 0.15s' }}>
...
}.finally(() => setListingsLoading(false)); }, [tab, user]);

  useEffect(() => {
    if (tab !== 'users' || !user || user.role !== 'admin') return;
    setUsersLoading(true);
    api.admin.users().then(d => setUsers(d || [])).catch(() => {}).finally(() => setUsersLoading(false));
  }, [tab, user]);
import { useAuthStore } from '../../lib/store';
import { api } from '../../lib/api';
