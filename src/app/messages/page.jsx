...
export default function MessagesPage() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', gap: 0, height: 'calc(100vh - 120px)', background: '#0a0e1a', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
      <div style={{ width: 280, borderRight: '1px solid rgba(255,255,255,0.06)', flexShrink: 0, overflowY: 'auto' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: 24, textAlign: 'center', color: '#4a5568', fontSize: 13 }}>No conversations yet.<br/><br/>Browse listings and click<br/>💬 Message Seller to start.</div>
        ) : convos.map(c => (
      ...
      {!selected ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4a5568', fontSize: 14 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#111620' }}>
            ...
            <a href={'/listing/'+selected.listing_id} style={{ fontSize: 11, color: '#3b82f6', textDecoration: 'none' }}>{selected.listing_title}</a>
          </div>
          ...
          <div style={{ padding: 12, borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 8 }}>
            ...
            <button onClick={send} disabled={sending || !input.trim()} style={{ background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: sending ? 'not-allowed' : 'pointer', opacity: sending || !input.trim() ? 0.5 : 1 }}>
              {sending ? '...' : 'Send'}
            </button>
          </div>
        </div>
