  ...
  return (
    <div style={{maxWidth:680,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24,flexWrap:'wrap',gap:12}}>
        <h1 style={{fontSize:22,fontWeight:800,color:'#e8eaf0',letterSpacing:'-0.02em'}}>My Orders</h1>
        <div style={{display:'flex',background:'#111620',border:'1px solid rgba(255,255,255,0.08)',borderRadius:8,overflow:'hidden'}}>
          {['buyer','seller'].map(r=>(
            <button key={r} onClick={()=>setRole(r)} style={{padding:'7px 16px',fontSize:13,fontWeight:500,cursor:'pointer',border:'none',background:role===r?'#3b82f6':'transparent',color:role===r?'#fff':'#8892a4'}}>
        ...
        {orders.map(order=>{
          ...
          return (
            <div key={order.id} style={{background:'#111620',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,overflow:'hidden'}}>
              <button onClick={()=>setOpen(exp?null:order.id)} style={{width:'100%',display:'flex',alignItems:'center',gap:12,padding:'16px 18px',textAlign:'left',background:'none',border:'none',cursor:'pointer'}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:'#4a5568'}}>{new Date(order.created_at).toLocaleDateString('en-US',{day:'numeric',month:'long',year:'numeric'})}</div>
                </div>
                ...
                <div style={{padding:'0 18px 18px',borderTop:'1px solid rgba(255,255,255,0.04)'}}>
                  ...
                  {order.status==='escrow_locked'&&role==='buyer'&&(
                    <div style={{display:'flex',gap:8}}>
                      <button onClick={()=>api.orders.confirm(order.id).then(()=>api.orders.mine(role).then(setOrders))}
                        style={{flex:1,background:'rgba(16,185,129,0.1)',color:'#34d399',border:'1px solid rgba(16,185,129,0.2)',borderRadius:8,padding:'10px',fontSize:13,fontWeight:500,cursor:'pointer'}}>
                        ✓ Received — Release Payment
                      </button>
                      <button onClick={()=>api.orders.dispute(order.id,{reason:'Issue'}).then(()=>api.orders.mine(role).then(setOrders))}
                        style={{background:'rgba(239,68,68,0.08)',color:'#f87171',border:'1px solid rgba(239,68,68,0.15)',borderRadius:8,padding:'10px 14px',fontSize:13,cursor:'pointer'}}>⚠</button>
                    </div>
                  )}
                </div>
              )}
