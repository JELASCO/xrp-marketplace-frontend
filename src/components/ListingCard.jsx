...
export default function ListingCard({ listing, isFavorited, onToggleFavorite }) {
  ...
  const emoji = GAME_EMOJIS[game] || 'Ã°ÂÂÂ®';
  ...
  return (
    <Link href={`/listing/${id}`} style={{textDecoration:'none',display:'block'}}>
      <div style={{
        ...
        {onToggleFavorite && <button onClick={(e)=>{e.preventDefault();e.stopPropagation();onToggleFavorite(id);}} style={{position:'absolute',top:8,right:8,zIndex:10,background:'rgba(0,0,0,0.5)',border:'none',borderRadius:'50%',width:30,height:30,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all 0.15s'}} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.2)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}><span style={{fontSize:16,color:isFavorited?'#f87171':'var(--text2)'}}>{isFavorited?'Ã¢ÂÂ¤Ã¯Â¸Â':'Ã°ÂÂ¤Â'}</span></button>}
        <div style={{height:130,background:'var(--surface2)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}>
          ...
          {is_featured && <div style={{position:'absolute',top:8,right:8,background:'rgba(245,158,11,0.2)',color:'#fbbf24',borderRadius:20,padding:'2px 8px',fontSize:11,fontWeight:600}}>Ã¢ÂÂ Featured</div>}
        </div>
        <div style={{padding:'12px 14px'}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:5,fontSize:11,color:'var(--text3)'}}>
              ...
              <span style={{maxWidth:70,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{username}{is_verified && <span title='Verified Seller' style={{marginLeft:4,fontSize:10,background:'rgba(16,185,129,0.15)',color:'var(--green)',borderRadius:4,padding:'1px 4px',fontWeight:700}}>Ã¢ÂÂ</span>}{is_verified && <span title='Verified Seller' style={{marginLeft:4,fontSize:10,background:'rgba(16,185,129,0.15)',color:'var(--green)',borderRadius:4,padding:'1px 4px',fontWeight:700,letterSpacing:'0.02em'}}>Ã¢ÂÂ Verified</span>}</span>
              {reputation_score > 0 && <span style={{color:'var(--amber)'}}>Ã¢ÂÂ{Number(reputation_score).toFixed(1)}</span>}
            </div>
          </div>
        </div>
      </div>
