...
export const api = {
  ...
  listings: { list: (p={}) => request('/listings?' + new URLSearchParams(p)), get: (id) => request(`/listings/${id}`), view: (id) => request('/listings/' + id + '/view', { method: 'POST' }), create: (d) => request('/listings', { method: 'POST', body: d }), update: (id,d) => request(`/listings/${id}`, { method: 'PATCH', body: d }), remove: (id) => request(`/listings/${id}`, { method: 'DELETE' }) },
